...
static void glade_project_options_show_file_selection (GtkWidget *widget,
						       gpointer data);
static void glade_project_options_set_filename (GtkWidget *widget,
						GladeProjectOptions *options);
...
{
  static guint glade_project_options_type = 0;
...
#ifdef USE_GNOME
  options->ok_button = gnome_stock_button (GNOME_STOCK_BUTTON_OK);
#else
  options->ok_button = gtk_button_new_with_label (_("OK"));
#endif
...
#ifdef USE_GNOME
  options->cancel_button = gnome_stock_button (GNOME_STOCK_BUTTON_CANCEL);
#else
  options->cancel_button = gtk_button_new_with_label (_("Cancel"));
#endif
...
glade_project_options_generate_name (GladeProjectOptions *options)
{
  ...
  directory = gtk_entry_get_text (GTK_ENTRY (options->directory_entry));
...
glade_project_options_generate_program_name (GladeProjectOptions *options)
{
  ...
  project_name = gtk_entry_get_text (GTK_ENTRY (options->name_entry));
...
glade_project_options_generate_xml_filename (GladeProjectOptions *options)
{
  ...
  program_name = gtk_entry_get_text (GTK_ENTRY (options->program_name_entry));
					   ...
					   gpointer   data)
{
  ...
  if (options->filesel == NULL)
    {
      options->filesel = gtk_file_selection_new (_("Select the Project Directory"));
      gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION (options->filesel)->ok_button),
			  "clicked",
			  GTK_SIGNAL_FUNC (glade_project_options_set_filename),
			  options);

      gtk_signal_connect_object (GTK_OBJECT (GTK_FILE_SELECTION (options->filesel)->cancel_button),
				 "clicked",
				 GTK_SIGNAL_FUNC (glade_util_close_window),
				 GTK_OBJECT (options->filesel));
      gtk_signal_connect (GTK_OBJECT (options->filesel), "delete_event",
			  GTK_SIGNAL_FUNC (glade_util_close_window_on_delete),
			  NULL);
      gtk_signal_connect (GTK_OBJECT (options->filesel), "key_press_event",
		      GTK_SIGNAL_FUNC (glade_util_check_key_is_esc),
		      GINT_TO_POINTER (GladeEscCloses));
    }

  filename = gtk_entry_get_text (GTK_ENTRY (options->directory_entry));
  if (glade_util_file_exists (filename))
    {
      gtk_file_selection_set_filename (GTK_FILE_SELECTION (options->filesel),
				       filename);
    }
  if (GTK_IS_WINDOW (options))
    gtk_window_set_transient_for (GTK_WINDOW (options->filesel),
				  GTK_WINDOW (options));
  gtk_widget_show (options->filesel);
  gdk_window_show (options->filesel->window);
  gdk_window_raise (options->filesel->window);
}


static void
glade_project_options_set_filename (GtkWidget *widget,
				    GladeProjectOptions *options)
{
  gchar *filename;

  filename = gtk_file_selection_get_filename (GTK_FILE_SELECTION (options->filesel));
